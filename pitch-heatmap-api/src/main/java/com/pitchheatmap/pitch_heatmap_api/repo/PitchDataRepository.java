...
public interface PitchDataRepository extends JpaRepository<PitchData, PitchId>{
    ...
    @Query(value = "SELECT new com.pitchheatmap.pitch_heatmap_api.dto.GameSummary(p.gamePk, MIN(p.gameDate), MIN(p.homeTeam), MIN(p.awayTeam), COUNT(p)) "
            + "FROM PitchData p "
            + "WHERE p.pitcher = :mlbamId AND p.gameYear = :season AND p.gameType = 'R' "
            + "GROUP BY p.gamePk "
